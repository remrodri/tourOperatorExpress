...
export class DateRangeRepository implements IDateRangeRepository {
  async findByIdDB(
    ...
    if (session) {
      return await DateRangeModel.findById(id).session(session).exec();
    }
    return await DateRangeModel.findById(id).exec();
  }
  async updateDateRangeDB(
    id: string,
    dto: UpdateDateRangeDto
  ): Promise<IDateRange | null> {
    const updatedDateRange = await DateRangeModel.findByIdAndUpdate(id, dto, {
      new: true,
    });
    if (!updatedDateRange) {
  ...
  async addTourPackageIdToDateRange(
    ...
    const updatedDateRange = await DateRangeModel.findByIdAndUpdate(
      id,
  ...
  async createDB(
    ...
    // console.log("savedDateRange::: ", savedDateRange);
